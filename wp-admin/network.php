...
function network_domain_check() {
	global $wpdb;
	if ( $wpdb->get_var( "SELECT  \"tablename\" FROM \"pg_tables\" WHERE \"tablename\" = '$wpdb->site'" ) )
...
function allow_subdirectory_install() {
	...
	$post = $wpdb->get_row( "SELECT \"ID\" FROM $wpdb->posts WHERE post_date < (NOW() - INTERVAL '1 MONTH') AND post_status = 'publish'" );
	if ( empty( $post ) )
		return true;
