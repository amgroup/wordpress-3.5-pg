...
class WP_User_Query {
	...
	function query() {
		...
		if ( is_array( $qv['fields'] ) || 'all' == $qv['fields'] ) {
			$this->results = $wpdb->get_results(preg_replace('/SQL_CALC_FOUND_ROWS/','',"SELECT $this->query_fields $this->query_from $this->query_where $this->query_orderby $this->query_limit"));
		} else {
			$this->results = $wpdb->get_col(preg_replace('/SQL_CALC_FOUND_ROWS/','',"SELECT $this->query_fields $this->query_from $this->query_where $this->query_orderby $this->query_limit"));
		}

		if ( $qv['count_total'] )
			//$this->total_users = $wpdb->get_var( apply_filters( 'found_users_query', 'SELECT FOUND_ROWS()' ) );
                           
                        //$hdl_db = $wpdb->query(preg_replace('/SQL_CALC_FOUND_ROWS/','',"SELECT $this->query_fields $this->query_from $this->query_where"));
                        $wpdb->query(preg_replace('/SQL_CALC_FOUND_ROWS/','',"SELECT $this->query_fields $this->query_from $this->query_where"));
                        /*
                        if (!empty($hdl_db)){
                            $this->total_users = pg_num_rows($hdl_db);
                        }  else {
                            $this->total_users = 0;
                        } 
                         * 
...
function count_users($strategy = 'time') {
	...
	if ( 'time' == $strategy ) {
		...
		$row = $wpdb->get_row( "SELECT $select_count, COUNT(*) FROM $wpdb->usermeta WHERE meta_key = '{$blog_prefix}capabilities'", ARRAY_N );
                //echo "SELECT $select_count, COUNT(*) FROM $wpdb->usermeta WHERE meta_key = '{$blog_prefix}capabilities'";
