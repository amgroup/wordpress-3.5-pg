...
*/
	...
	function _real_escape( $string ) {
			...
			return pg_escape_string( $string, $this->dbh );
		else
	...
	function db_connect() {
		...
		$this->is_postgre = true;

		$new_link = defined( 'PGSQL_CONNECT_FORCE_NEW_WP' ) ? PGSQL_CONNECT_FORCE_NEW : 0;
		//$client_flags = defined( 'MYSQL_CLIENT_FLAGS' ) ? MYSQL_CLIENT_FLAGS : 0;

		if ( WP_DEBUG ) {
			$this->dbh = pg_connect( "host=$this->dbhost user=$this->dbuser password=$this->dbpassword dbname=$this->dbname", $new_link);
		} else {
			$this->dbh = @pg_connect( "host=$this->dbhost user=$this->dbuser password=$this->dbpassword dbname=$this->dbname", $new_link);
		}
	...
	function query( $query ) {
		...
		if ( preg_match( '/^\s*(create|alter|truncate|drop)\s/i', $query ) ) {
			...
			if ( preg_match( '/^\s*(insert|replace)\s/i', $query ) ) {
                //PG TODO:
				$this->insert_id = $this->get_var($this->_real_escape("SELECT lastval()"));
			}
...
//			$wheres[] = "$field = $form";
	 ...
	 */
	function db_version() {
		return preg_replace( '/[^0-9.].*/', '', pg_version( $this->dbh ) );
	}
}
