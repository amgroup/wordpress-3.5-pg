	...
	function query( $query_vars ) {
		...
		if ( $groupby )
			$groupby = 'GROUP BY ' . $groupby;
                ...
                //postgresql
                if (strpos('COUNT(*)', $fields) !== false && empty($groupby) ){
                    $query = "SELECT $fields FROM $wpdb->comments $join WHERE $where $limits";
                }else{
...
function wp_allow_comment($commentdata) {
	...
	$dupe = "SELECT \"comment_ID\" FROM $wpdb->comments WHERE \"comment_post_ID\" = '$comment_post_ID' AND comment_parent = '$comment_parent' AND comment_approved != 'trash' AND ( comment_author = '".$wpdb->_real_escape($comment_author)."' ";
	if ( $comment_author_email )
		$dupe .= "OR comment_author_email = '".$wpdb->_real_escape($comment_author_email)."' ";
	$dupe .= ") AND comment_content = '".$wpdb->_real_escape($comment_content)."' LIMIT 1";
...
function wp_update_comment_count_now($post_id) {
	...
	$new = (int) $wpdb->get_var( $wpdb->prepare("SELECT COUNT(\"comment_ID\") FROM $wpdb->comments WHERE \"comment_post_ID\" = %d AND comment_approved = '1'", $post_id) );
