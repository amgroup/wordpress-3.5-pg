...
class WP_Query {
	...
	function get_posts() {
		...
		if ( empty($q['nopaging']) && !$this->is_singular ) {
			...
			if ( empty($q['offset']) ) {
				$pgstrt = ' OFFSET '.($page - 1) * $q['posts_per_page'];
			} else { // we're ignoring $page and using 'offset'
				$q['offset'] = absint($q['offset']);
				$pgstrt = ' OFFSET '. $q['offset'];
			}
			$limits = 'LIMIT '. $q['posts_per_page'] . $pgstrt ;
		}
		...
		if ( 'ids' == $q['fields'] ) {
			...
			$this->set_found_posts( $q, $limits, $request_for_count );
		...
		if ( $split_the_query ) {
			// First get the IDs and then fill in the objects
                        ...
                        $request_for_count =  "SELECT COUNT(1) FROM (SELECT $wpdb->posts.\"ID\"   FROM $wpdb->posts $join WHERE true $where " . ($groupby ? ' GROUP BY ' . $groupby : '') . ") AS tmp_tbl";
	...
	function set_found_posts( $q, $limits, $request_for_count = null ) {
		...
		if ( (! empty( $limits )) && (!empty($request_for_count))){
			//$this->found_posts = $wpdb->get_var( apply_filters_ref_array( 'found_posts_query', array( 'SELECT FOUND_ROWS()', &$this ) ) );
                        $this->found_posts = $wpdb->get_var( apply_filters_ref_array( 'found_posts_query', array($request_for_count , &$this ) ) );
                }    
		else
