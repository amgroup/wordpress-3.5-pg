...
function add_option( $option, $value = '', $deprecated = '', $autoload = 'yes' ) {
	...
	if ( is_array( $notoptions ) && isset( $notoptions[$option] ) ) {
		...
		UPDATE \"$wpdb->options\" SET \"option_value\" = %s, \"autoload\" = %s WHERE \"option_name\" = %s RETURNING \"option_name\";
		", $value, $autoload, $option)
        );

        $result = $wpdb->query( $wpdb->prepare( "
		INSERT INTO \"$wpdb->options\" (\"option_name\", \"option_value\", \"autoload\")
		(SELECT \"option_name\", \"option_value\", \"autoload\" FROM (
		    SELECT %s::text AS \"option_name\", %s::text AS \"option_value\", %s::text AS \"autoload\" 
		) n
		WHERE NOT EXISTS (
		    SELECT 1 FROM \"$wpdb->options\" o WHERE o.\"option_name\" = n.\"option_name\"
