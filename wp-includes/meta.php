...
function add_metadata($meta_type, $object_id, $meta_key, $meta_value, $unique = false) {
	...
	if ( $unique && $wpdb->get_var( $wpdb->prepare(
		"SELECT COUNT(1) FROM $table WHERE meta_key = %s AND $column = %d",
		$meta_key, $object_id ) ) )
		return false;
	...
	*/
	...
	function get_sql( $type, $primary_table, $primary_id_column, $context = null ) {
		...
		foreach ( $queries as $k => $q ) {
			...
			$meta_type = isset( $q['type'] ) ? strtoupper( $q['type'] ) : 'VARCHAR';

			if ( 'NUMERIC' == $meta_type )
				$meta_type = 'INTEGER';
			elseif ( ! in_array( $meta_type, array( 'VARCHAR', 'BINARY', 'CHAR', 'DATE', 'DATETIME', 'DECIMAL', 'SIGNED', 'TIME', 'UNSIGNED' ) ) )
				$meta_type = 'VARCHAR';
