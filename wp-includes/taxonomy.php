...
function get_objects_in_term( $term_ids, $taxonomies, $args = array() ) {
	...
	$where[] = "tt.taxonomy IN (" . (empty($taxonomies) ? "NULL,''" : "'".implode( "','", $taxonomies )."'"). ")";
	$where[] = "tt.term_id IN (" . (empty($term_ids) ? "NULL": implode( ',', $term_ids )) . ")";

	if( ! empty($where) ) { $where = "WHERE " . implode( " AND ", $where ); } else { $where = ''; }

	$object_ids = $wpdb->get_col("SELECT tr.object_id FROM $wpdb->term_relationships AS tr INNER JOIN $wpdb->term_taxonomy AS tt ON tr.term_taxonomy_id = tt.term_taxonomy_id $where ORDER BY tr.object_id $order");
	...
	public function get_sql( $primary_table, $primary_id_column ) {
		...
		foreach ( $this->queries as $index => $query ) {
			...
			if ( 'IN' == $operator ) {
				...
				$join .= $i ? " AS $alias" : '';
				$join .= " ON ($primary_table. ".stripslashes($primary_id_column)."   = $alias.object_id)";
				...
				$terms = (empty( $terms ) ? "NULL" : implode( ',', $terms ));
				...
				$where[] = "(
					SELECT COUNT(1)
					FROM $wpdb->term_relationships
					WHERE term_taxonomy_id IN ($terms) AND object_id = $primary_table.$primary_id_column
				) = $num_terms";
			}
		...
		switch ( $query['field'] ) {
			...
			case 'name':
				$terms = $wpdb->get_col( "
					...
					WHERE taxonomy = '{$query['taxonomy']}' AND $wpdb->terms.{$query['field']} IN (" .(empty( $query['terms'] ) ? "NULL" : ("'". implode( "','", array_map( 'sanitize_title_for_query', $query['terms'] ) ) ."'")). ")"
				);
				break;
			case 'term_taxonomy_id':
				$terms = $wpdb->get_col( "
					SELECT $resulting_field
					FROM $wpdb->term_taxonomy WHERE term_taxonomy_id IN (" . (empty( $query['terms'] ) ? 'NULL' : implode( ',', array_map( 'intval', $query['terms'] ) )) . ")"
				);
				break;
			default:
				$terms = $wpdb->get_col( "
					SELECT $resulting_field
					FROM $wpdb->term_taxonomy
					WHERE taxonomy = '{$query['taxonomy']}' AND term_id IN (" . (empty( $query['terms'] ) ? 'NULL' : implode( ',', array_map( 'intval', $query['terms'] ) )) . ")"
				);
		}
...
function wp_delete_object_term_relationships( $object_id, $taxonomies ) {
	...
	foreach ( (array) $taxonomies as $taxonomy ) {
		...
		$in_tt_ids = (empty($tt_ids) ? "NULL" : implode(",", $tt_ids));
...
function wp_get_object_terms($object_ids, $taxonomies, $args = array()) {
	...
	$where = array();
	$where[] = "tt.taxonomy IN (" . (empty($taxonomies) ? "NULL,''" : ("'".implode("','", $taxonomies)."'")) . ")";
	$where[] = "tr.object_id IN (" . (empty($object_ids) ? "NULL" : implode(',', $object_ids)) . ")";
	...
	$query = "SELECT $select_this FROM $wpdb->terms AS t INNER JOIN $wpdb->term_taxonomy AS tt ON tt.term_id = t.term_id INNER JOIN $wpdb->term_relationships AS tr ON tr.term_taxonomy_id = tt.term_taxonomy_id $where $orderby $order";
	...
	if ( 'all' == $fields || 'all_with_object_id' == $fields ) {
		...
		$terms = $wpdb->get_col("SELECT tr.term_taxonomy_id FROM $wpdb->term_relationships AS tr INNER JOIN $wpdb->term_taxonomy AS tt ON tr.term_taxonomy_id = tt.term_taxonomy_id $where $orderby $order");
	}
...
function wp_set_object_terms($object_id, $terms, $taxonomy, $append = false) {
	...
	if ( ! $append ) {
		...
		if ( $delete_terms ) {
			$in_delete_terms = (empty($delete_terms) ? "NULL" : "'" . implode("', '", $delete_terms) . "'");
	...
	if ( ! $append && isset($t->sort) && $t->sort ) {
		...
		foreach ( $tt_ids as $tt_id )
			if ( in_array($tt_id, $final_tt_ids) )
				$values[] = array( $object_id, $tt_id, ++$term_order );
		if ( $values )
			foreach( $values as $value )
				if ( false === $wpdb->query( $wpdb->prepare("UPDATE $wpdb->term_relationships SET term_order=%s WHERE object_id=%s AND term_taxonomy_id=%s", $value[2], $value[0], $value[1] ) ) )
					if ( false === $wpdb->query( $wpdb->prepare("INSERT INTO $wpdb->term_relationships (object_id, term_taxonomy_id, term_order) VALUES (%s,%s,%s)", $value[0], $value[1], $value[2]) ) )
...
function clean_term_cache($ids, $taxonomy = '', $clean_taxonomy = true) {
	...
	if ( empty($taxonomy) ) {
		...
		$tt_ids = (empty($tt_ids) ? "NULL" :implode(',', $tt_ids));
...
function _pad_term_counts(&$terms, $taxonomy) {
	...
	$results = $wpdb->get_results("SELECT object_id, term_taxonomy_id FROM $wpdb->term_relationships INNER JOIN $wpdb->posts ON object_id = \"ID\" WHERE term_taxonomy_id IN (" . (empty($term_ids) ? "NULL" : implode(',', array_keys($term_ids))). ") AND post_type IN (".(empty($object_types) ? "NULL,''" : ("'".implode("','", $object_types)."'")).") AND post_status = 'publish'");
...
function _update_post_term_count( $terms, $taxonomy ) {
	...
	foreach ( (array) $terms as $term ) {
		...
		if ( $object_types )
			$count += (int) $wpdb->get_var( $wpdb->prepare( "SELECT COUNT(*) FROM $wpdb->term_relationships, $wpdb->posts WHERE $wpdb->posts.\"ID\" = $wpdb->term_relationships.object_id AND post_status = 'publish' AND post_type IN (" . (empty($object_types) ? "NULL,''": ("'".implode("', '", $object_types )."'")) . ") AND term_taxonomy_id = %d", $term ) );
