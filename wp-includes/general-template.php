...
function wp_get_archives($args = '') {
	...
	if ( 'monthly' == $type ) {
		//$query = "SELECT YEAR(post_date) AS \"year\", MONTH(post_date) AS \"month\", count(\"ID\") as posts FROM $wpdb->posts $join $where GROUP BY YEAR(post_date), MONTH(post_date) ORDER BY post_date $order $limit";
            $query = "SELECT extract(year from \"post_date\") AS \"year\", extract(month from \"post_date\") AS \"month\", count(\"ID\") as posts FROM $wpdb->posts $join $where GROUP BY year,month ORDER BY year,month $order $limit";
            //echo $query;
