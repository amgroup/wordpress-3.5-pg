...
function get_pages($args = '') {
	...
	if ( !empty($number) and $number != 'ALL' )
		$query .= ' LIMIT ' . (int) $number;

	$offset = (int) $offset;
        if ( !empty($offset) and $offset > 0 )
            $query .= ' OFFSET ' . $offset;
...
function wp_delete_auto_drafts() {
	...
	$old_posts = $wpdb->get_col( "SELECT \"ID\" FROM $wpdb->posts WHERE post_status = 'auto-draft' AND ( NOW() - INTERVAL '7 DAY' ) > post_date" );
